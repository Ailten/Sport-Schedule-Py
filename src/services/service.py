...
from ..models import makeSession
...
class Service(ABC):
    ...
    @classmethod
    def getService(cls, session: Session=Depends(makeSession)):
        return cls(session_db=session)

    # ---> CRUD.
    def create(self, model_to_add) -> bool:
        self._session_db.add(model_to_add)
        self._session_db.commit()
        self._session_db.refresh(model_to_add)
        return True
    ...
    def update(self, model_to_update) -> bool:
        self._session_db.commit()
        self._session_db.refresh(model_to_update)
        return True

    def delete(self, model_to_delete) -> bool:
        self._session_db.delete(model_to_delete)
        self._session_db.commit()
        return True
